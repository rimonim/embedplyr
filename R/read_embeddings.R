...
read_embeddings_binary <- function(path, words = NULL, .keep_missing = FALSE) {
	...
	available_words <- words[!embedding_not_found]

	# initialize matrix
	emb <- matrix(NA_real_, nrow = length(available_words), ncol = dimensions, dimnames = list(available_words))
	...
	if (.keep_missing) {
		out <- matrix(NA_real_, nrow = length(words), ncol = dimensions, dimnames = list(words))
		out[!embedding_not_found, ] <- emb
	}else{
		out <- emb
		if (length(out) == 0L) return(numeric())
	}
